    ...
    public function reinvestBonus(Request $request): JsonResponse
    {
        ...
        if (!$package) {
            $package = StakingPackage::create([
                ...
                'daily_rate' => \App\Support\Rate::dailyRateFromApy((float) config('staking.apy.discovery', 0.04), (string) config('staking.rate_mode', 'simple')),
                'min_amount' => 0,
                'max_amount' => null,
                'duration_days' => config('staking.bonus.discovery_days', 90),
                'level' => 'discovery',
                'is_active' => true,
